...
#include "Terramancy.hpp"

Terramancy::Terramancy(std::string const& name,short int const& level,float const& currentXP
		,float const& sUM,float const& sIM,float const& sIO):Skill(name,level,currentXP,sUM,sIM,sIO){
	setExpNextLevel();

}

void Terramancy::setExpNextLevel(){
	m_expNextLev = m_skillImpMult*(float)pow((float)m_level,1.95) + m_skillImpOffset;
}

void Terramancy::passLevel(){
...
void Terramancy::useSkill(float const& baseXP){
